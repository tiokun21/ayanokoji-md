...
const name = process.env.NAME || "Mnuvq"
...
module.exports = async (sock, msg) => {
    ...
    switch (command) {
        ...
        case "waifu": {
            try {
                ...
                await sock.sendMessage(from, {
                    ...
                    caption: "More than one waifu will ruin your laifu"
                }, {
                    quoted: msg
                });
            } catch (e) {
        ...
        case 's': {
            if (text) {
                anu = text.split('|')
                packName = anu[0] !== '' ? anu[0] : "🤖"
                authorName = anu[1] !== '' ? anu[1] : "Mnuvq"
            } else {
                packName = "🤖";
                authorName = "Mnuvq";
            }
        ...
        case "img": {
            if (!text) {
                reply(`usage: search and get image of given term! \n eg. *${prefix}imgs iron man*`)
                return;
            }
            try {
                if (!text) {
        ...
        case 'owner': {
            const vcard = 'BEGIN:VCARD\n' +
                ...
                'FN: *Mnuvq* \n' +
                'ORG:;\n' +
                'TEL;type=CELL;type=VOICE;waid=9372521352:+9372521352\n' +
                'END:VCARD'
            let buttonMessaged = {
                contacts: { displayName: "Mnuvq", contacts: [{ vcard }] },
                contextInfo: {
                    externalAdReply: {
                        title: "Mnuvq",
                        body: 'Touch here.',
            ...
            sock.sendMessage(from, buttonMessaged, {
                quoted: msg,
            });
        }
            break
        ...
        case "remove": {
            ...
            return reply("✔ member removed from group!");
        ...
        case "event": {
            ...
            switch (text) {
                ...
                case "off": {
                    ...
                    return reply("Event has been trurned off here")
                }
                    break
                default:
                    reply(`No such option please choose between [ on / off ]`)
        ...
        case "ban": {
            ...
            if (ban.includes(user)) return reply("Alrady banned")
...
URL:~> ${con.content_urls.mobile.page}
        ...
        case "tagall": {
            ...
            if (!text || !(text == "--admin")) {
                if (!isGroupAdmins) return reply('Its an admin command')
                let mention = []
                let result = `*${groupName}-Members tagged by ${senderName}*\n\n`
                ...
                let mention = []
                let result = `*${groupName}-Members tagged only admins by ${senderName}*\n\n`
...
Spoiler: ${data.spoiler}`
        ...
        default:
            if (body.startsWith(prefix)) {
                ...
                var diffuser = senderNumber;
                await axios.get(`http://api.brainshop.ai/get?bid=170434&key=FrCpqU3jDT1i4KG7&uid=[${diffuser}]&msg=[${body}]`)
                    .then((response) => {
                        // console.log(response);
                        const tet = response.data.cnt
                        reply(tet)
                    })
                    .catch((err) => {
                        reply(`✖  An error occurred.`)
